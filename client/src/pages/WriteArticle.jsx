...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) =>{
    ...
    try{
      setLoading(true)
      const prompt = `Write a detailed, well-structured article of approximately ${selectedLength.length} words about ${input}. Include an introduction, headings, examples, and a conclusion.`
  ...
  return (
    <div className = 'h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit = {onSubmitHandler} className = 'w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <br/>
        <button disabled = {loading} className = 'w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 rounded-lg mt-6 text-sm cursor-pointer'>
          {
